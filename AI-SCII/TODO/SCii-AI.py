...
class MyBot(BotAI):  # inherits from BotAI
    async def on_step(self, iteration: int):  # method called every step of the game
        ...
        if self.townhalls:  # does AI have a townhall?
            ...
            if nexus.is_idle and self.can_afford(UnitTypeId.PROBE) and supply_remaining > 4 and self.units(
                    UnitTypeId.PROBE).amount < 26:
                nexus.train(UnitTypeId.PROBE)

            # if AI has less than 5 Pylons, build one close to nexus:
            elif not self.structures(UnitTypeId.PYLON) and self.already_pending(
                    UnitTypeId.PYLON) == 0:  # if AI doesn't have a pylon
            ...
            # if AI has less than 5 Cybernetics cores, build one close to nexus:
                ...
                if self.can_afford(UnitTypeId.PYLON):  # and AI can afford one
                    ...
                    target_pylon = self.structures(UnitTypeId.PYLON).closest_to(
                        self.enemy_start_locations[0])  # find closest pylon to enemy start location
            ...
            # if AI doesn't have a forge, build one close to nexus:
                ...
                if self.can_afford(UnitTypeId.FORGE):  # and AI can afford one:
                    ...
                    await self.build(UnitTypeId.FORGE,
                                     near=self.structures(UnitTypeId.PYLON).closest_to(nexus))  # if so,

            # if AI has less than three canons, build one close to nexus:
            elif self.structures(UnitTypeId.FORGE).ready and self.structures(
                    UnitTypeId.PHOTONCANNON).amount < 3:  # if AI has Forge and less than 3 cannons
